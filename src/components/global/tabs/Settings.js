...
class Settings_ extends React.Component {
  ...
  render() {
    ...
    let discordbtn = (
      <button className="btn" onClick={this.GenerateToken}>
    ...
    if (this.props.settings.link != undefined) {
      ...
      discordtext =
        "Room linked to channel #" +
        link.channelname +
        " on server " +
        link.servername;
      discordbtn = (
        <button className="btn" onClick={this.DisconnectDiscord}>
          Remove link to channel
        </button>
      );
    }
        ...
        <div className="row">
          <div className="col" id="settings_usertable_col">
            <h4 className="d-flex align-items-center">
              <span className="flex-grow-1">Manage users</span>
              <button
                type="button"
                className="btn"
                onClick={() =>
                  this.setState({
                    showAuthenticatedUsers: !this.state.showAuthenticatedUsers,
                  })
                }
              >
                {this.state.showAuthenticatedUsers ? "Hide" : "Show"}{" "}
                Authenticated
              </button>
              <button
                type="button"
                ...
                onClick={() =>
                  this.setState({
                    showBannedUsers: !this.state.showBannedUsers,
                  })
                }
              >
          ...
          <div className="col">
            ...
            <div>
              ...
              <p>
                Follow this link:{" "}
                <a href="https://discordapp.com/api/oauth2/authorize?client_id=530789519360917514&permissions=0&scope=bot">
                  https://discordapp.com/api/oauth2/authorize?client_id=530789519360917514&permissions=0&scope=bot
                </a>
              </p>
              <h5> - Town hall/Fort notifications</h5>
              <p>
                If you have a discord channel linked to your bot, you can make
                bot notify you of events in desired town halls/forts by
                selecting a town/fort and clicking the 'Notify in Discord'
                checkbox.
              </p>
              <h5> - Operation timers</h5>
              <p>
                Discord emits a message when an operation is scheduled and when
                there is 1 hour left until the operation start.
              </p>
              <h5> - Tech tree</h5>
              <p>
                Discord emits a message when a research has been completed (this
                is based on user actions and is not linked to the game).
                Alternatively you can type !techtree and get all techs currently
                being researched (not empty and not full techs)
              </p>
            </div>
          </div>
        </div>
...
class ManagementlistUnit extends React.Component {
  ...
  render() {
    ...
    if (this.props.user.id.includes("anonymous")) {
      avatar = "/img/glitch/dasd.jpg?1556805827222";
    }
    for (var i = 1; i < 5; i++) {
        ...
        <td>
          ...
          {this.props.user.rank == 5 && (
            <p>
              ...
              >
                <img className="approve_img" src="/img/glitch/checkmark.png" />
              </button>
              <button
                className="approvebtn"
                onClick={() => this.HandleRankChange(4)}
              >
                <img className="approve_img" src="/img/glitch/XIconSmall.png" />
              </button>
            </p>
          )}
        </td>
...
class DiscordPopover extends React.Component {
      ...
      <Popover.default
        ...
        content={
          <div id="settings_popover">
            ...
            <p>
              2. Set up a channel for the bot and give the bot rights to read,
              write, embed and upload
            </p>
            <p>
            ...
            <p>
              4. If you have manage permissions in the channel, the channel will
              be successfully linked to the room
            </p>
          </div>
        }
      >
        <button
          id="settings_discord_explain_btn"
          onClick={() =>
            this.setState({ isPopoverOpen: !this.state.isPopoverOpen })
          }
        >
          <img
            id="settings_discord_explain_img"
            src="/img/glitch/disc.png?v=1554634446293"
          />
        </button>
      </Popover.default>
    );
