...
exports.updateObject = function (packet) {
  ...
  if (packet.type.includes("misc")) {
    ...
    let obj = {};
    if (packet.type.includes("requests")) {
      obj = {
        ...type[packet.key],
        ...packet.object,
        wip: [
      ...
      obj = { ...type[packet.key], ...packet.object };
    }
    type[packet.key] = obj;
    sql
...
exports.toggleSecure = function (globalid, data) {
  ...
  if (data == 1) {
    sql
      .prepare("DELETE FROM userglobal WHERE userid LIKE ? AND globalid = ?;")
      .run("anonymous%", globalid);
...
exports.clearRoom = function (globalid) {
  ...
  sql
    .prepare(
      "INSERT OR REPLACE INTO global (id, admin, settings, techtree,refinery, production, storage, stockpiles,fobs, requests, misc, arty,squads,logi,events) VALUES (@id, @admin, @settings, @techtree, @refinery, @production, @storage, @stockpiles,@fobs, @requests, @misc, @arty,@squads, @logi, @events);",
    )
    .run(e);
};
...
exports.insertrelation = sql.prepare(
  "INSERT OR REPLACE INTO userglobal (id, userid, globalid, rank, role) VALUES (?, ?, ?, ?, ?);",
);
